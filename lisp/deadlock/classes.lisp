...
(declare-around-list-checker quick quick-requests request)
(declare-around-list-checker quick quick-positions position)
(declare-around-list-checker quick quick-subaccount-instrument list)
   
(defclass subaccount()
  ((name :initform "subaccount" :initarg :name :reader subaccount-name)
   (free-money :initform nil :initarg :free-money :accessor subaccount-free-money :documentation "свободные средства на счете"))
  (:documentation "класс описывающий субсчет"))
(defmethod (setf subaccount-free-money) :around (val (obj subaccount))
  (if (> val 0)
      (call-next-method val obj)
      (error (make-condition 'incorrect-arguments :format-control "you can not set ~a to subaccount free-money" :format-arguments (list val)))))
...
(defclass request ()
  ((direction :initarg :direction :reader request-direction :documentation  "must be `:buy' or `:sell'")
   ...
   (execution-date :initform nil :accessor request-execution-date :documentation "дата исполнения заявки")
   (overtime-date :initform nil :accessor request-overtime-date :documentation "дата просрочки исполнения заявки")
   (state :initform :awaiting :initarg :state :reader request-state :documentation "can be `:awaiting' `:executed' or `:overtimed'")
   (set-callback :initform nil :initarg :set-callback :accessor request-set-callback :documentation "коллбэк на установку заявки в торговую систему")
   (execute-callback :initform nil :initarg :execute-callback :accessor request-execute-callback :documentation "коллбэк на исполнение заявки")
   (overtime-callback :initform nil :initarg :overtime-callback :accessor request-overtime-callback :documentation "коллбэк на просрочку заявки"))
  (:documentation "заявка на покупку - продажу"))
(defmethod shared-initialize :after ((obj request) slot-names &rest initargs &key)
  (declare (ignore slot-names initargs))
  (let ((direction (request-direction obj))
        (state (request-state obj))
        (set-call (request-set-callback obj))
        (exec-call (request-execute-callback obj))
        (over-call (request-overtime-callback obj)))
    (declare (type (member :sell :buy) direction)
             (type (member :awaiting :executed :overtimed) state)
             (type (or function null) set-call exec-call over-call))))
(defmethod (setf request-ttl) :around (val (obj request))
  (declare (type (or number list symbol) val))
  (call-next-method val obj))
(macrolet ((set-rqdate (accessor value)
             `(defmethod (setf ,accessor) :around (val (obj request))
                (declare (type number val))
                (call-next-method val obj)
                (setf (request-state obj) ,value))))
  (set-rqdate request-execution-date :executed)
  (set-rqdate request-overtime-date :overtimed))
(macrolet ((setf-to-function (accessor)
             `(defmethod (setf ,accessor) :around (val (obj request))
                (declare (type (or nil function) val))
                (call-next-method val obj))))
  (setf-to-function request-execute-callback)
  (setf-to-function request-overtime-callback)
  (setf-to-function request-set-callback))
...
(defclass open-position ()
  ((code :initform nil :initarg :code :reader open-position-code)
   ...
   (deals :initform nil :initarg :deals :accessor open-position-deals :documentation "список сделок по данной позиции")
   (money-holds :initform nil :initarg :money-holds :accessor open-position-money-holds :documentation "список удержаний денежных средств по позиции")
