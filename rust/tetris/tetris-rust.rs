...
use crossterm::{
    ...
    execute, terminal::{Clear, ClearType, EnterAlternateScreen, LeaveAlternateScreen, SetSize},
...
impl Tetromino {
    ...
    fn can_move(&self, board: &[Vec<Option<Color>>], dx: i32, dy: i32) -> bool {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                if ny >= 0 {
                    if let Some(Some(_)) = board.get(ny as usize).and_then(|row| row.get(nx as usize)) {
                        return false;
                    }
                }
            }
    ...
    fn can_rotate(&self, board: &[Vec<Option<Color>>]) -> bool {
        ...
        for y in 0..BOARD_HEIGHT {
            for x in 0..BOARD_WIDTH {
                ...
                if let Some(color) = cell {
                    execute!(stdout, SetBackgroundColor(color), Print("  "), SetBackgroundColor(Color::Reset))?;
                }
            }
        }
